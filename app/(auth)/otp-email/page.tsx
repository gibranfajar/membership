...
export default function Otp() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(
        `https://golangapi-j5iu.onrender.com/send-mail-otp-validasi-verify?`,
        data,
  ...
  const handleRecodeOTP = async () => {
    ...
    const response = await axios.post(
      `https://golangapi-j5iu.onrender.com/send-mail-otp-validasi?userAccount=${getEmail}&memberID=${getMember}`
    );
      ...
      <div className="flex flex-col items-center w-full max-w-md bg-white md:rounded-lg min-h-screen">
        ...
        <div className="flex flex-col justify-center items-center m-8">
          ...
          <p className="text-xs text-center my-6 fontMon">
            kode OTP akan dikirm ke email anda
          </p>
